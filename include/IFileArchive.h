...
public:
	...
	or 0 on failure. */
	virtual std::shared_ptr<IReadFile> createAndOpenFile(const path& filename) =0;
	...
	virtual std::shared_ptr<IReadFile> createAndOpenFile(u32 index) =0;
...
public:
	...
	\return True if file seems to be loadable. */
	virtual bool isALoadableFileFormat(std::shared_ptr<io::IReadFile> file) const =0;
	...
	virtual std::shared_ptr<IFileArchive> createArchive(const path& filename, bool ignoreCase, bool ignorePaths) const =0;
	...
	\param ignorePaths Files are searched for without checking for the directories
	\return Pointer to newly created archive, or 0 upon error. */
	virtual std::shared_ptr<IFileArchive> createArchive(std::shared_ptr<io::IReadFile> file, bool ignoreCase, bool ignorePaths) const =0;
};
